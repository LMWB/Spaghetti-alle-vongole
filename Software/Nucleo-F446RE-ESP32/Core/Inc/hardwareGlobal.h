...
#define UART_SEND_IR(string, size)    HAL_UART_Transmit(&UART_INTERNET_HANDLER, string, size, HAL_MAX_DELAY)

#define UART_IR_READ_BYTE_IRQ(buffer)			HAL_UART_Receive_IT(&UART_INTERNET_HANDLER, buffer, 1)
#define UART_IR_READ_LINE_IRQ(buffer, size)	HAL_UARTEx_ReceiveToIdle_DMA(&UART_INTERNET_HANDLER, buffer, size);
