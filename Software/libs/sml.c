...
static uint16_t ir_buffer_head = 0;						// acts like a kind of fifo with wrap around

static uint8_t uart_terminal_rx_buffer[	uart_rx_buffer_size];
static bool sml_telegram_read_complete = false;

// off-line sample to test search byte algorithm
...
void sml_print_bytes_on_terminal(void){
	// regular printf() does not work, it swallows chars!
	char b[8];
	uint8_t s = 0;
	s = sprintf(b, "%02X ", uart_IR_rx_buffer[ir_buffer_head]);
...
// Function to search for a byte pattern in an array (generic)
...
// Function to search for a byte pattern in an array (SML-optimized)
...
// testing search algorithm off-line
...
bool sml_check_for_start_sequence(uint8_t *buffer) {
	if (	buffer[ir_buffer_head] == 0x01 && buffer[ir_buffer_head - 1] == 0x01 &&
			buffer[ir_buffer_head - 2] == 0x1B && buffer[ir_buffer_head - 3] == 0x1B) {
